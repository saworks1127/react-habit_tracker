...
export function parseHHmm(s: string): HHmm {
    // 00:00〜23:59 のみ許容
    if (!/^\d{1,2}:\d{2}$/.test(s)) throw new Error("Invalid HH:mm");
    const [hh, mm] = s.split(":").map(Number);
    console.log(`Parsing HH:mm: ${hh}:${mm}`);
    if (hh < 0 || hh > 23 || mm < 0 || mm > 59) throw new Error("Invalid HH:mm");
...
export function todayDow(d = new Date()): DayOfWeek {
    return d.getDay() as DayOfWeek; // 0..6
}